...
def mat2axisangle(mat):
    """
    Convert a batch of rotation matrices to axis-angle representation.
    Args:
        mat: size = [B, 3, 3]
    Returns:
        axis-angle representation -- size = [B, 3]
    """
    # Convert rotation matrix to quaternion
    q_w = 0.5 * torch.sqrt(torch.clamp(1 + mat[:, 0, 0] + mat[:, 1, 1] + mat[:, 2, 2], min=0.0))
    q_x = torch.sign(mat[:, 2, 1] - mat[:, 1, 2]) * 0.5 * torch.sqrt(torch.clamp(1 + mat[:, 0, 0] - mat[:, 1, 1] - mat[:, 2, 2], min=0.0))
    q_y = torch.sign(mat[:, 0, 2] - mat[:, 2, 0]) * 0.5 * torch.sqrt(torch.clamp(1 - mat[:, 0, 0] + mat[:, 1, 1] - mat[:, 2, 2], min=0.0))
    q_z = torch.sign(mat[:, 1, 0] - mat[:, 0, 1]) * 0.5 * torch.sqrt(torch.clamp(1 - mat[:, 0, 0] - mat[:, 1, 1] + mat[:, 2, 2], min=0.0))

    # Convert quaternion to axis-angle representation
    angles = 2 * torch.atan2(torch.sqrt(q_x**2 + q_y**2 + q_z**2), q_w)
    normalized_axis = torch.stack([q_x, q_y, q_z], dim=-1)
    normalized_axis = normalized_axis / (torch.norm(normalized_axis, dim=-1, keepdim=True) + 1e-8)
    axis_angle = angles.unsqueeze(-1) * normalized_axis
